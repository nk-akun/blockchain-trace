...
public class FarmerController extends BaseController {
  ...
  @Value("${fdfs.address}") private String address;
  ...
  public AjaxResult
  getAllDriverByDeptId(@PathVariable("deptId") String deptId) {
  ...
  public AjaxResult imageUpload(@RequestBody CropsImageVo cropsImageVo)
      throws FileNotFoundException {
    String imageBase64 =
        cropsImageVo.getImageBase64().replace("data:image/jpeg;base64,", "");
    File file = Base64Util.base64ConvertFile(imageBase64);
    StorePath storePath =
        this.fastFileStorageClient.uploadImageAndCrtThumbImage(
            new FileInputStream(file), file.length(), "jpg", null);
    return AjaxResult.success("http://" + address + '/' +
  ...
  public AjaxResult
  getCropsByUsername(@PathVariable("username") String username) {
