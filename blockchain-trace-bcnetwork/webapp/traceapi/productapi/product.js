...
//溯源过程记录
router.get("/queryProductInfoByCropsId", async function (req, res) {
  try {
    ...
    const request = {
      ...
      chaincodeId: "productinfocc",
      fcn: "queryProductInfoByCropsId",
      args: [req.query.id],
    };
...
//原料厂商质检信息
router.post("/createProductInfo", async function (req, res) {
  console.log("productInfo    " + JSON.stringify(req.body.productInfoArray));
  try {
    var state_store = await Fabric_Client.newDefaultKeyValueStore({
      path: store_path,
    });
    ...
    const request = {
      chaincodeId: "productinfocc",
      fcn: "createProductInfo",
      txId: tx_id,
      args: req.body.productInfoArray,
...
//溯源操作
router.get("/queryOperationByCropsId", async function (req, res) {
  try {
    var state_store = await Fabric_Client.newDefaultKeyValueStore({
      path: store_path,
    });
    ...
    const request = {
      //targets : --- letting this default to the peers assigned to the channel
      chaincodeId: "productprocesscc",
      fcn: "queryOperationByCropsId",
      args: [req.query.id],
    };
...
//添加操作
router.post("/createOperation", async function (req, res) {
  console.log("productInfo    " + JSON.stringify(req.body.operationArray));
  try {
    var state_store = await Fabric_Client.newDefaultKeyValueStore({
      path: store_path,
    });
