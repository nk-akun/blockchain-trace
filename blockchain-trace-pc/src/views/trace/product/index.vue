  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="cropsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            v-show="scope.row.machingStatus !== 1 && scope.row.machingStatus !== 2"
            size="mini"
            type="text"
            @click="taskDistribution(scope.row)"
          >任务分发</el-button>
          <el-button
            v-show="scope.row.machingStatus !== null"
            size="mini"
            type="text"
            @click="queryTaskByCropsId(scope.row)"
          >查看任务分发信息</el-button>
          <el-button
            v-show="scope.row.productWriteStatus !== 1"
            size="mini"
            type="text"
            @click="addThing(scope.row)"
          >填写产品基本信息</el-button>
          <el-button
            v-show="scope.row.productWriteStatus !== null"
            size="mini"
            type="text"
            @click="queryProductInfo(scope.row)"
          >查看产品基本信息</el-button>
          ...
          <el-button
            v-show="scope.row.outFactoryStatus === null"
            size="mini"
            type="text"
            @click="outFactory(scope.row)"
          >产品出库</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog center title="任务分发" :visible.sync="open" width="700px" append-to-body>
      <el-form ref="form" :model="form" label-width="80px">
        <el-form-item label="任务标题">
          <el-input v-model="form.taskName" placeholder="请输入任务标题"></el-input>
        </el-form-item>
        <el-form-item label="任务分工内容">
          <el-input v-model="form.taskRemark" type="textarea" placeholder="请输入内容"></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog center title="生产基本信息添加" :visible.sync="addThingDialog" width="700px" append-to-body>
      <el-form ref="form" label-width="80px" :model="recordForm">
        <el-row>
          <el-col :span="12">
            <el-form-item label="品名" prop="nickName">
              <el-input v-model="recordForm.ProductName" placeholder="请输入" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="配料" prop="nickName">
              <el-input v-model="recordForm.MixedIngredients" placeholder="请输入" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="负责人" prop="nickName">
              <el-input v-model="recordForm.Leader" placeholder="请输入" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="电话" prop="nickName">
              <el-input v-model="recordForm.LeaderTel" placeholder="请输入" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="生产车间" prop="nickName">
              <el-input v-model="recordForm.Workshop" placeholder="请输入" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="生产工时" prop="nickName">
              <el-input v-model="recordForm.WorkHours" placeholder="请输入" />
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <el-dialog center title="员工工作量证明" :visible.sync="staffWorkDialog" width="1200px" append-to-body>
      <el-row :gutter="8">
        <el-col
          :span="6"
          v-for="(info,index) in operationList"
          :key="index"
  ...
  methods: {
    ...
    submitProductInfo() {
      ...
      this.$httpBlock
        ...
        .then((res) => {
          if (res.status === 200) {
            updateProductWriteStatus(this.cropsInfos.cropsId)
              .then((res) => {
    ...
    queryTaskByCropsId(row) {
      queryTaskByCropsId(row.cropsId)
        .then((res) => {
